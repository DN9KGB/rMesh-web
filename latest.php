...
if ($json) {
    ...
    logOtaEvent($call, 'version_check', $version, $latest, '');
    if ($version && $version !== $latest) {
        logOtaEvent($call, 'update_found', $version, $latest, '');
    } elseif ($version && $version === $latest) {
        logOtaEvent($call, 'no_update', $version, $latest, '');
    }
} else {
    ...
    require_once __DIR__ . '/ota_log_helper.php';
    logOtaEvent($call, 'version_check_failed', $version, '', 'GitHub API nicht erreichbar');
}
